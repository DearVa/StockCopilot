...
public class EastMoneyKLinesDataSource(IConfiguration configuration) : IKLinesDataSource
{
    ...
    {
        Converters = { new KLineJsonConverter(Fields2) }
    };
        ...
        AdjustmentType adjustmentType = AdjustmentType.None)
    {
        ...
        var kLineResponse = await JsonSerializer.DeserializeAsync<KLineResponse>(stream, JsonSerializerOptions);
        return kLineResponse?.Data?.KLines ?? Array.Empty<KLine>();
    }
}